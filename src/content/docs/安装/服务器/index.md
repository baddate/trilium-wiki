...
sidebar:
  order: 0
---
本页介绍在您自己的服务器上安装Trilium。如果您想设置[同步](./同步)，或者要将其用作Trilium的Web版本（可以从任何地方访问它），则可能需要这样做。
...
对于服务器安装，您可能要配置例如port或[TLS配置](./tls配置)。这是在Trilium配置文件中完成的，默认情况下它在[数据目录](/备份/数据目录)中的`config.ini`文件中。
