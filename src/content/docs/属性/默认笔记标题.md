...
对于此用例，Trilium（从 v0.52 开始）支持`#titleTemplate`[标签](./属性)。您可以为给定笔记创建此类标签，为其分配一个值，在创建子笔记时，此值将用作默认标题。与其他标签一样，您可以使其可继承以递归方式应用，甚至可以将其放在根笔记上，以便在所有位置全局应用它。
